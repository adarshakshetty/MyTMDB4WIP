...
class WatchlistMoviesAdapter @Inject constructor() :
    ...
    object DiffCallback : DiffUtil.ItemCallback<WatchlistMovie>() {
        override fun areItemsTheSame(old: WatchlistMovie, new: WatchlistMovie): Boolean =
            old.id == new.id

        override fun areContentsTheSame(old: WatchlistMovie, new: WatchlistMovie): Boolean =
            old == new
    }
