...
class GeneralNetworkExceptionCodeTest(
    ...
    fun `calling getFromCode with NetworkException code, it returns correct GeneralNetworkExceptionCode`() {
        // Act
        val actualGeneralNetworkExceptionCode =
            GeneralNetworkExceptionCode.getFromCode(networkExceptionCode)
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any?>> {
            return listOf(
                arrayOf(
                    NetworkException.CODE_PARSING_EXCEPTION,
                    GeneralNetworkExceptionCode.PARSING_EXCEPTION,
                ),
                arrayOf(
                    NetworkException.CODE_IO_EXCEPTION,
                    GeneralNetworkExceptionCode.IO_EXCEPTION,
                ),
                arrayOf(
                    NetworkException.CODE_HTTP_EXCEPTION,
                    GeneralNetworkExceptionCode.HTTP_EXCEPTION,
                ),
                arrayOf(
                    NetworkException.CODE_TIMEOUT_EXCEPTION,
                    GeneralNetworkExceptionCode.TIMEOUT_EXCEPTION,
                ),
                arrayOf(
                    NetworkException.CODE_OTHER_EXCEPTION,
                    GeneralNetworkExceptionCode.OTHER_EXCEPTION,
                ),
                arrayOf(-1, null),
            )
        }
